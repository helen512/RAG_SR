...
def run_polopt_agent(env_fn, 
    ...
    o, r, d, c, ep_ret, ep_cost, ep_len = gymnasium_reset_compat(env), 0, False, 0, 0, 0, 0
    cur_penalty = 0
    cum_cost = 0
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            # Step in environment
            o2, r, d, info = gymnasium_step_compat(env, a)

            # If requested, replace stored action with environment-corrected action
            a_to_store = a
            if getattr(env, 'use_corrected_action_for_training', False):
                corrected = info.get('training_action', info.get('certified_action'))
                if corrected is not None:
                    corrected = np.asarray(corrected, dtype=np.float32)
                    if act_shape == ():
                        corrected = np.array(corrected).astype(np.float32).reshape(())
                    else:
                        corrected = corrected.reshape(act_shape)
                    logp_input = {x_ph: o[np.newaxis], a_ph: corrected[np.newaxis]}
                    logp_eval = sess.run(logp, feed_dict=logp_input)
                    logp_t = float(np.squeeze(logp_eval))
                    a_to_store = corrected
            ...
            if agent.reward_penalized:
                ...
                buf.store(o, a_to_store, r_total, v_t, 0, 0, logp_t, pi_info_t)
            else:
                buf.store(o, a_to_store, r, v_t, c, vc_t, logp_t, pi_info_t)
            ...
            if terminal or (t==local_steps_per_epoch-1):
                ...
                # Reset environment
                o, r, d, c, ep_ret, ep_len, ep_cost = gymnasium_reset_compat(env), 0, False, 0, 0, 0, 0
