...
class CBFWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if violated:
            ...
            modified_reward = -200.0 + reward  # Heavy negative penalty for violation
            terminated = True  # Terminate episode immediately on violation
            done = True
        else:
            # Use original reward structure when no violation
...
def train_ppo_with_cbf(counter: ConstraintViolationCounter, c1: float, c2: float):
    ...
    def env_fn():
        ...
        env = Monitor(cbf_wrapper_instance, log_dir, info_keywords=("original_reward", "violation", "last_10_dist_sum"))
        return env
    ...
    plt.plot(counter.timesteps, counter.returns, alpha=0.3, label='Episode Reward')
    
    # Calculate rolling mean
    if len(counter.returns) >= 50:
        rolling_mean = pd.Series(counter.returns).rolling(window=50).mean()
        plt.plot(counter.timesteps, rolling_mean, color='red', linewidth=2, label='Rolling Mean (50)')
    ...
    print(f"Saved reward plot to {os.path.join(log_dir, 'reward_vs_timesteps.png')}")
    plt.close()
...
def main():
    ...
    # c1_values = [0.2, 0.5, 0.8, 1, 2, 4, 6, 8, 10]
    # c2_values = [10, 20, 30, 50, 70, 80, 100]
    
    # results = []
    
    # for c1 in c1_values:
    #     for c2 in c2_values:
    #         print(f"\nRunning experiment with c1={c1}, c2={c2}")
    #         counter = ConstraintViolationCounter()
    #         summary = train_ppo_with_cbf(counter, 0, c1, c2)
    ...
    #             'total_violations': summary.get('violation_episodes', 0),
    #             'correction_rate': summary.get('correction_rate', 0.0),
    #             'avg_correction': summary.get('avg_correction', 0.0)
    #         })
    ...
    #         c1_unique = sorted(list(set(r['c1'] for r in results)))
    #         c2_unique = sorted(list(set(r['c2'] for r in results)))
            
    #         violation_grid = np.zeros((len(c1_unique), len(c2_unique)))
    #         correction_grid = np.zeros((len(c1_unique), len(c2_unique)))
            
    #         for r in results:
    #             i = c1_unique.index(r['c1'])
    #             j = c2_unique.index(r['c2'])
    #             violation_grid[i, j] = r['total_violations'] # or violation_rate
    #             correction_grid[i, j] = r['correction_rate']
            
    #         # Plot Violations Heatmap
    #         plt.figure(figsize=(12, 10))
    #         plt.imshow(violation_grid, interpolation='nearest', cmap='viridis')
    #         plt.colorbar(label='Total Violation Episodes')
    #         plt.xticks(np.arange(len(c2_unique)), c2_unique)
    #         plt.yticks(np.arange(len(c1_unique)), c1_unique)
    #         plt.xlabel('c2')
    #         plt.ylabel('c1')
    #         plt.title('Parameter Sweep: Total Violations')
            
    #         # Add text annotations
    #         for i in range(len(c1_unique)):
    #             for j in range(len(c2_unique)):
    #                 plt.text(j, i, f'{violation_grid[i, j]:.0f}', 
    #                          ha='center', va='center', color='white')
            
    #         save_path = os.path.join(RUN_DIR, f'parameter_sweep_violations_{save_index}.png')
    #         plt.savefig(save_path)
    #         print(f"Saved violations heatmap to {save_path}")
    #         plt.close()
            
    #         # Plot Correction Rate Heatmap
    #         plt.figure(figsize=(12, 10))
    #         plt.imshow(correction_grid, interpolation='nearest', cmap='plasma')
    #         plt.colorbar(label='Correction Rate')
    #         plt.xticks(np.arange(len(c2_unique)), c2_unique)
    #         plt.yticks(np.arange(len(c1_unique)), c1_unique)
    #         plt.xlabel('c2')
    #         plt.ylabel('c1')
    #         plt.title('Parameter Sweep: Correction Rate')
    #          # Add text annotations
    #         for i in range(len(c1_unique)):
    #             for j in range(len(c2_unique)):
    #                 plt.text(j, i, f'{correction_grid[i, j]:.2f}', 
    #                          ha='center', va='center', color='white')
            
    #         save_path = os.path.join(RUN_DIR, f'parameter_sweep_corrections_{save_index}.png')
    #         plt.savefig(save_path)
    #         print(f"Saved corrections heatmap to {save_path}")
    #         plt.close()
