...
def save_all_parameters(save_index):
    """
    Save all configuration parameters to a txt file with the given save_index.
    """
    # Get DQN parameters from the function that builds DQN
    dqn_params = get_dqn_params()
    
    params = {
        # Basic configuration
        "SEED": SEED,
        "TOTAL_TIMESTEPS_BASE": TOTAL_TIMESTEPS_BASE,
        "TOTAL_TIMESTEPS_SYM": TOTAL_TIMESTEPS_SYM,
        "RUN_DIR": RUN_DIR,
        "save_index": save_index,
        
        # Haystack/Retrieval configuration
        "QDRANT_COLLECTION": QDRANT_COLLECTION,
        "QDRANT_PERSIST": QDRANT_PERSIST,
        "BM25_CACHE_JSONL": str(BM25_CACHE_JSONL),
        ...
        "sr_populations": 10,
        "sr_maxsize": 20,
        "sr_dataset_size": 6000,
        
        # Environment thresholds and normalization
        "x_normalization_divisor": "x_threshold",
        "theta_normalization_divisor": "theta_threshold",
        "x_dot_tanh_scale": 2.0,
        "theta_dot_tanh_scale": 3.0,
    ...
    # Add DQN parameters from the function
    params.update(dqn_params)
    
    # Create the parameters file path
    params_path = os.path.join(RUN_DIR, f"parameters_{save_index}.txt")
    
    # Write parameters to file
    with open(params_path, 'w') as f:
        f.write("=" * 50 + "\n")
        f.write(f"PARAMETERS FOR RUN {save_index}\n")
        f.write("=" * 50 + "\n\n")
        
        # Group parameters by category
        dqn_param_names = list(dqn_params.keys())
        categories = {
            "Basic Configuration": ["SEED", "TOTAL_TIMESTEPS_BASE", "TOTAL_TIMESTEPS_SYM", "RUN_DIR", "save_index"],
            "Retrieval/Haystack": ["QDRANT_COLLECTION", "QDRANT_PERSIST", "BM25_CACHE_JSONL", "EMBED_MODEL", "RERANK_MODEL", "dense_k", "sparse_k", "final_k"],
            "DQN Hyperparameters": dqn_param_names,
            "Symbolic Regression": ["sr_niterations", "sr_populations", "sr_maxsize", "sr_dataset_size"],
            "Environment/Normalization": ["x_normalization_divisor", "theta_normalization_divisor", "x_dot_tanh_scale", "theta_dot_tanh_scale"],
            "LLM Configuration": ["llm_model", "llm_temperature", "llm_max_tokens"],
            "Visualization": ["moving_avg_window"]
        }
...
def get_dqn_params():
    """Return DQN parameters as a dictionary for saving and consistency."""
    return { 
        "learning_rate":1e-3,                 # or schedule to 1e-4
        "buffer_size":50_000,
        "learning_starts":2_000,
        "batch_size":128,
        "train_freq":256, "gradient_steps":256,  # ratio ~ 1.0
        "target_update_interval":1000,
        "exploration_fraction":0.08,
        "exploration_final_eps":0.002,
        "policy_kwargs":dict(net_arch=[256,256]),  # (dueling stays on by default)
...
def build_dqn(env):
    ...
    return DQN(
        params["policy"], env, seed=SEED, verbose=params["verbose"],
        ...
        replay_buffer_kwargs=dict(handle_timeout_termination=params["handle_timeout_termination"]),
...
if __name__ == "__main__":
    ...
    # Save all parameters at the beginning of the run
    save_all_parameters(save_index)
