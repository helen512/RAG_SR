...
class CBFSafetyFilter:
    ...
    def certify_action(self, state: np.ndarray, uncertified_action: float) -> Tuple[float, bool]:
        ...
        if cbf_constraint >= -1e-8:  # Tighter tolerance for safety
            ...
            correction_magnitude = abs(uncertified_action - (-1*uncertified_action))
...
def extract_training_data(ppo_dir: str, ppo_lag_dir: str, cpo_dir: str, cbf_dir: str,
    ...
    ppo_progress['ViolationsPerEpoch'] = ppo_violations_per_epoch[:]
    ppo_lag_progress['ViolationsPerEpoch'] = ppo_lag_violations_per_epoch[:]
    cpo_progress['ViolationsPerEpoch'] = cpo_violations_per_epoch[:]
    cbf_progress['ViolationsPerEpoch'] = cbf_violations_per_epoch[:]
