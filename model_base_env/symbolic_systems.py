...
This file is a modified version of the SymbolicModel class in the safe-control-gym repository.
It is used to create a symbolic model for the base environment.

fc: Continuous-time dynamics - computes the instantaneous derivative x_dot = f(x,u) at a single point.
fd: Discrete one-step integrator - advances the state from time t to t+dt by numerically integrating x_dot = f(x,u).
    ...
    def __init__(self,
        ...
        if params is not None:
            for name, param in params.items():
                assert name not in self.__dict__ # Check if the parameter is already defined
                self.__dict__[name] = param 
        # Variable dimensions.
        self.nx = self.x_sym.shape[0] # Number of state variables
        self.nu = self.u_sym.shape[0] # Number of input variables
        self.ny = self.y_sym.shape[0] # Number of output variables
        # Setup cost function.
        self.cost_func = cost['cost_func']
        # print(self.cost_func)
        self.Q = cost['vars']['Q'] # Cost weight matrix for state
        self.R = cost['vars']['R'] # Cost weight matrix for input
        self.Xr = cost['vars']['Xr'] # Reference state/desired state
        self.Ur = cost['vars']['Ur'] # Reference input
    ...
    def setup_linearization(self):
        ...
        self.fd_linear_func = cs.integrator(
            'fd_linear', self.integration_algo, {
                'x': self.x_eval,
                'p': cs.vertcat(self.u_eval, self.x_sym, self.u_sym), # Concatenate input and state variables
